...
const deliArrC = [];
const totalIncome = [];
const cutPercentage = 0.03;
$.getJSON(URLCurYrData, function (queryData) {
    queryData.forEach(data => {
        totalArrC.push({x: data.Month, y: data.Total});
        totalIncome.push({x: data.Month, y: (data.Total * cutPercentage).toFixed(2)})
...
dateElement.addEventListener('change', (event) => {
    ...
    $.getJSON(URLGetDailyRev, function (dailyRevenue) {
        ...
        dailyRevenue.forEach(hourData => {
            hourlyRevenue.push({
                x: `${hourData.timeInterval}-${parseInt(hourData.timeInterval) + 1}`,
                y: hourData.totRevenue
            });
    ...
    $.getJSON(URLGetDailyOrd, function (dailyOrders) {
        ...
        dailyOrders.forEach(hourOrder => {
            hourlyOrders.push({
                x: `${hourOrder.timeInterval}-${parseInt(hourOrder.timeInterval) + 1}`,
                y: hourOrder.totOrders
            });
        });
...
$('#SalesDate2').val(new Date().toDateInputValue());
const fireEvent = new Event('change');
dateElement.dispatchEvent(fireEvent);
...
const urlMonthNewUsers = host + '/dashboard/staff/getnewusercount';
...
monthInput.addEventListener('change', (event) => {
    ...
    const URLGetMonthNewUsers = urlMonthNewUsers.concat("?SalesMonth1=" + event.target.value + "-01")
    console.log(URLGetMonthNewUsers)
    ...
    $.getJSON(URLGetMonthNewUsers, function (monthNewUsers) {
        const totalNew = parseInt(monthNewUsers[0].NewCount) + parseInt(monthNewUsers[1].NewCount) + parseInt(monthNewUsers[2].NewCount) + parseInt(monthNewUsers[3].NewCount) + parseInt(monthNewUsers[4].NewCount);
        $('#TNew').val(totalNew);
        $('#NCust').val(monthNewUsers[0].NewCount);
        $('#NDStaff').val(monthNewUsers[1].NewCount);
        $('#NDRider').val(monthNewUsers[2].NewCount);
        $('#NShop').val(monthNewUsers[3].NewCount);
        $('#NSStaff').val(monthNewUsers[4].NewCount);
    })
...
const month = (curDate.getMonth() + 1).toLocaleString('en-US', {minimumIntegerDigits: 2, useGrouping: false});

$('#SalesMonth1').val(year + "-" + month);
const fireEvent2 = new Event('change');
monthInput.dispatchEvent(fireEvent2);
